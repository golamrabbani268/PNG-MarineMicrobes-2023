...
ggsave(ra_barplot_its_mangrove_host_phylum, filename = "./Outputs/relativeAbundanceBarplot-mangrove-host-phylum-PNG-ITS.svg", dpi = 300, width = 12, height = 10)
...
ggsave(ra_barplot_its_mangrove_host_phylum_full, filename = "./Outputs/relativeAbundanceBarplot-mangrove-host-phylum-PNG-ITS-full.svg", dpi = 300, width = 12, height = 10)
...
diversity <- data.frame(
                  ...
                  simpson = vegan::diversity(otu_table(psra.png.its.mangrove, taxa_are_rows = FALSE), index = "simpson"),
                  evenness = evenness(otu_table(psra.png.its.mangrove, taxa_are_rows = FALSE)),
                  richness = richness(otu_table(psra.png.its.mangrove, taxa_are_rows = FALSE)))
...
ggplot(diversity, aes(x = hostSection, y = richness.observed)) + 
  ...
  scale_color_manual(values = COLOR_SITE) +
  facet_wrap(vars(host))
ggsave(filename = "./Outputs/richnessObserved-hostSection-mangrove-PNG-ITS.svg", dpi = 300, width = 12, height = 10)
...
renameTaxafromASV <- function(ASVID, psq = ps.png.its){
  psq <- psq %>% tax_fix()
  renamedTaxa <- NULL
  for (i in ASVID){
...
svglite("./Outputs/vennDiagram-hostSection-mangrove-AA-PNG-ITS.svg")
venn_hostSection_AA
dev.off()
...
svglite("./Outputs/vennDiagram-hostSection-mangrove-SA-PNG-ITS.svg")
venn_hostSection_SA
dev.off()
...
diff_taxa_host_ancombc <- res[res$`passed_ss_hostSonneratia alba`, "taxon"]
```
...
sum(!is.na(fungi_guilds$guild)[taxa_sums(ps.png.its.mangrove)>0]) / nrow(fungi_guilds[taxa_sums(ps.png.its.mangrove)>0,]) # Proportion of taxa assigned a guild. Total: 0.6145276. Mangrove: 0.6545226
