...
emptySamples <- sample.names[out[,2] == 0]
for(emptySample in emptySamples){
  file.create(filtFs[[emptySample]])
  file.create(filtRs[[emptySample]])
}
...
```{r}
errF <- learnErrors(filtFs[!(sample.names %in% emptySamples)], multithread=TRUE) 
errR <- learnErrors(filtRs[!(sample.names %in% emptySamples)], multithread=TRUE) 
...
for (sam in sample.names){
  if(sam %in% emptySamples){
    cat("Sample", sam, "is empty. Not processing.")
    dadaFs[[sam]] <- NA
    dadaRs[[sam]] <- NA
    mergers[[sam]] <- NA
  }else{
...
```{r}
seqtab <- makeSequenceTable(mergers[!(sample.names %in% emptySamples)])
dim(seqtab) # 367 24989
...
track <- cbind(out, sapply(dadaFs[!(sample.names %in% emptySamples)], getN), sapply(dadaRs[!(sample.names %in% emptySamples)], getN), sapply(mergers[!(sample.names %in% emptySamples)], getN), rowSums(seqtab.nochim))
...
```{r}
# Resume Point
seqtab.nochim <- readRDS("./Outputs/seqtabs/seqtab_nochim_SA-ITS1.RDS")
```
...
```{r}
# Resume Point
seqtab.nochim <- readRDS("./Outputs/seqtabs/seqtab_nochim_AA-ITS1.RDS")
```
...
```{r}
# Resume Point
seqtab.nochim <- readRDS("./Outputs/seqtabs/seqtab_nochim_EA-ITS1.RDS")
```
...
```{r}
# Resume Point
seqtab.nochim <- readRDS("./Outputs/seqtabs/seqtab_nochim_TH-ITS1.RDS")
```
