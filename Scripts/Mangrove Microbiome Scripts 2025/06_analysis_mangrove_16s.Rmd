...
```{r}
refseq_df <- data.frame(refseq(ps.png.bac.mangrove))
write.csv(refseq_df, file = "./Outputs/referenceSequence-mangrove-16s.csv")
```
...
ggsave(ra_barplot_bac_mangrove_host_phylum, filename = "./Outputs/relativeAbundanceBarplot-mangrove-host-phylum-PNG-16s.svg", dpi = 300, width = 12, height = 10)
...
ggsave(ra_barplot_bac_mangrove_host_phylum_full, filename = "./Outputs/relativeAbundanceBarplot-mangrove-host-phylum-PNG-16s-full.svg", dpi = 300, width = 12, height = 10)
...
### Get mean abundance by samples for each phylum
```{r}
library(microViz)

# Overall
psra.phylum <- tax_agg(tax_fix(psra.png.bac.mangrove), rank = "Phylum")
otu.phylum <- data.frame(otu_table(psra.phylum))
summary(otu.phylum)
sd(otu.phylum[,"Pseudomonadota"])
sd(otu.phylum[,"Bacteroidota"])
sd(otu.phylum[,"Actinobacteriota"])
sd(otu.phylum[,"Chloroflexi"])
 

# Fruit
psra.phylum <- tax_agg(tax_fix(psra.png.bac.mangrove.fruit), rank = "Phylum")
otu.phylum <- data.frame(otu_table(psra.phylum))
summary(otu.phylum)
sd(otu.phylum[,"Pseudomonadota"])
sd(otu.phylum[,"Bacteroidota"])
...
# Leaf
psra.phylum <- tax_agg(tax_fix(psra.png.bac.mangrove.leaf), rank = "Phylum")
otu.phylum <- data.frame(otu_table(psra.phylum))
summary(otu.phylum)
sd(otu.phylum[,"Pseudomonadota"])
sd(otu.phylum[,"Bacteroidota"])
...
# Pneumatophore
psra.phylum <- tax_agg(tax_fix(psra.png.bac.mangrove.pneumatophore), rank = "Phylum")
otu.phylum <- data.frame(otu_table(psra.phylum))
summary(otu.phylum)
sd(otu.phylum[,"Pseudomonadota"])
sd(otu.phylum[,"Bacteroidota"])
...
# Sediment
psra.phylum <- tax_agg(tax_fix(psra.png.bac.mangrove.sediment), rank = "Phylum")
otu.phylum <- data.frame(otu_table(psra.phylum))
summary(otu.phylum)
sd(otu.phylum[,"Pseudomonadota"])
sd(otu.phylum[,"Bacteroidota"])
...
diversity <- data.frame(
                  ...
                  richness = richness(otu_table(psra.png.bac.mangrove, taxa_are_rows = FALSE)))
...
ggplot(diversity, aes(x = hostSection, y = richness.observed)) + 
  ...
  scale_color_manual(values = COLOR_SITE) +
  facet_wrap(vars(host))
ggsave(filename = "./Outputs/richnessObserved-hostSection-mangrove-PNG-16s.svg", dpi = 300, width = 12, height = 10)
...
svglite("./Outputs/vennDiagram-hostSection-mangrove-AA-PNG-16S.svg")
venn_hostSection_AA
dev.off()
...
svglite("./Outputs/vennDiagram-hostSection-mangrove-SA-PNG-16S.svg")
venn_hostSection_SA
dev.off()
```
...
```{r}
library(corncob)
ps.png.bac.aa.pruned
ps.png.bac.aa.fruit
ps.png.bac.aa.leaf
ps.png.bac.aa.pneumatophore
ps.png.bac.aa.sediment
...
my.sites.environment.long <- pivot_longer(my.sites.environment, !Name, names_to = "Variable", values_to = "Value") %>% 
  group_by(Variable) %>%
  mutate(mean_value = mean(Value, na.rm = TRUE)) %>%
  ungroup()
ggplot(my.sites.environment.long) + 
  geom_point(aes(Name, Value)) +
  geom_boxplot(aes(y = Value), width = 0.2) +
  geom_hline(aes(yintercept = mean_value), linetype = 'dotted', colour = 'red') +
  facet_wrap(vars(Variable), scales = "free")

env_pca <- prcomp(my.sites.environment[,-1],
                  scale=TRUE)
PC <- as.data.frame(env_pca$x)
head(PC)
PC$Site <- my.sites.environment$Name

ggplot(PC) + geom_point(aes(x = PC1, y = PC2, color = Site))

fviz_pca_biplot(env_pca, 
                repel = TRUE,
                label = c("all"),
                col.var = "deepskyblue",
                title = "Environmental Variables by Site Biplot", geom="point")
```
...
ps.object <- psra.png.bac.mangrove.sediment
env.raw = read.csv("./sdmpredictorsData/PNG_env_extract_all.csv", row.names = 1)
...
env.data <- dplyr::left_join(meta.data, env.norm, by = join_by(site))
...
svg("./Outputs/RDA/model_1_env_correlations-mangrove-PNG-16s.svg") #EDIT
...
svg("./Outputs/RDA/model_1_env_correlations_selected-mangrove-PNG-16s.svg") #EDIT
pairs.panels(pred, scale=T)
dev.off()
...
screeplot(bac.rda) 
svg("./Outputs/RDA/model_1_RDA_screeplot-mangrove-PNG-16s.svg")
screeplot(bac.rda) 
dev.off()
...
write.table(cand, "./Outputs/RDA/model_1_RDA_OTUs-mangrove-PNG-16s.txt",  quote=F) #EDIT
cand
...
write.table(cand, "./Outputs/RDA/model_1_OTU_env_correlations-mangrove-PNG-16s.txt", quote = F) #EDIT
cand
...
colorScheme <- data.frame(
  ...
  Color = COLOR_21[1:15]#c('#00cccc', '#cc0066', '#ffcc66', '#6600cc') #EDIT. Should equal the number of unique predictors. Extra colors: 
)
...
plot(bac.rda, type="n", scaling=3, #xlim=c(-0.5,0.5), ylim=c(-0.5,0.5),
     main = "Mangrove Sediment Bacterial Predictors of Environment")
...
ggplot(subset(function_table.aa.png.16s.long, functionalGroup %in% significant_functional_groups)) + 
  geom_boxplot(aes(x = value, y = hostSection, color = site)) +
  facet_wrap(vars(functionalGroup), scales = "free")
```
